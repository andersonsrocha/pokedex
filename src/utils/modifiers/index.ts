...
export const rubberbandModifier: Modifier = ({ draggingNodeRect, transform, windowRect }) => {
...
function rubberbandIfOutOfBounds(position: number, min: number, max: number, elasticity = 0.3) {
  if (elasticity === 0) return clamp(position, min, max);
  if (position < min) return -rubberband(min - position, max - min, elasticity) + min;
  if (position > max) return +rubberband(position - max, max - min, elasticity) + max;
  return position;
}
